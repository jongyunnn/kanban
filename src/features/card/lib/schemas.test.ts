...
describe('Validation Schemas', () => {
  describe('cardFormSchema', () => {
    ...
    it('should fail when title is too long', () => {
      ...
      expect(result.success).toBe(false)
      if (!result.success) {
        expect(result.error.issues[0].message).toBe('카드 제목은 100자 이내로 입력해주세요.')
      }
    });
  });
  ...
  describe('columnTitleSchema', () => {
    it('should validate valid column title', () => {
      expect(columnTitleSchema.safeParse({ title: 'To Do' }).success).toBe(true)
    });

    it('should fail when column title is too long', () => {
      expect(columnTitleSchema.safeParse({ title: 'a'.repeat(51) }).success).toBe(false)
    });
  });
});
