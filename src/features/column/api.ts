...
export async function fetchColumns(): Promise<Column[]> {
  const response = await apiClient.get<ColumnsResponse, ColumnsResponse>(
    "/columns"
  );
...
export async function createColumn(data: CreateColumnRequest): Promise<Column> {
  const response = await apiClient.post<ColumnResponse, ColumnResponse>(
    "/columns",
    data
  );
...
export async function deleteColumn(
  id: string
): Promise<{ success: boolean; deletedCardsCount: number }> {
